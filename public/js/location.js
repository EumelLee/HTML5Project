...
//util.js에 require 함수 만들어져 있음
//googlemap api 끝나면 initmap 호출됨
...
function initMap(){
  // 1.1 구글맵 로딩 //getDocumentByID 대신 씀 
  var mapContainer = document.querySelector('#location_map');
	var mapOptions = {
    center: {lat: 37.527066, lng: 127.028437},
    zoom: 14
  };
  ...
  navigator.geolocation.getCurrentPosition(success, fail);
    ...
    new google.maps.Circle({
      ...
      strokeColor: 'blue',
      strokeOpacity: 0.2,//1: 완전 불투명 0: 완전투명
      fillColor: 'blue',
      fillOpacity: 0.1
    });
...
function addCouponToMap(){
  ...
  articleList.each(function(){
    ...
    // 3. 슬라이드 이벤트 추가  - >슬라이드가 새롭게 정리되었으니 다시 이벤트 등록 하도록
    setSlideEvent();
